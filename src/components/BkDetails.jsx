...
export default function BkDetails() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const amount = data.billamount - data.advance; // Calculate amount here
    const updatedFormData = { ...fdata, amount }; // Include amount in the form data
    axios
      .put(BASE_URL + 'api/bookings/complete/' + bid, updatedFormData)
      .then((resp) => {
        toast.success(resp.data);
        loadData();
      })
